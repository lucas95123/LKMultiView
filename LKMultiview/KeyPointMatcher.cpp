...
void getKeypointMatchesWithAKAZE(
	...
	akaze->detect(imageL, kptsL);
	akaze->detect(imageR, kptsR);
	akaze->compute(imageL, kptsL, descL);
	akaze->compute(imageR, kptsR, descR);
	...
	cout << "# matches from AKAZE = " << goodMatches.size() << endl;
}
