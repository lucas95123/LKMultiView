...
void readCameraParameters(vector<CameraParameter> &vecCameraParam, string path)
{
	...
	for (int i = 0; i < iSize; i++)
	{
		...
		cameraParam.setIntrinsicParam(k);
...
void readCameraPose(vector<CameraParameter>& vecCameraParam, string path)
{
	ifstream fin(path);
	if (!fin.is_open())
	{
		throw FileNotFoundException("Camera pose file does not exist");
	}
	...
	if (iSize<0)
	{
		throw InvalidFormatException("Invalid camera pose file format");
	}
	...
	for (int i = 0; i < iSize; i++)
	{
		...
		vecCameraParam[i].setExtrinsicParamR(r);
		vecCameraParam[i].setExtrinsicParamT(t);
		if (i == 0)
...
void showImage(string windowName,const Mat &img,int delay)
{
...
void showTwoImage(string windowName, const Mat &img1, const Mat &img2, bool isHorizontal, int delay)
{
	Mat visualization;
	if (isHorizontal)
		hconcat(img1, img2, visualization);
	else
		vconcat(img1, img2, visualization);
	showImage(windowName, visualization, delay);
}
